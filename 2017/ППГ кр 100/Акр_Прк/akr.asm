	...
	;Прикладна программа, яку повинен друкувати кожен студент та вивчити 
...
.code                                           ;Сегмент програми  
	   ...
	   local lop
	    ...
	    jg nx1		     ;Критерій ознаки порівняння Y < X
	    inc byte ptr [bx]+len    ;Дія обчисленняY:=Y + 1
	    jmp Whl		      ;Перехід на дію 
        nx1:ret                                  ;Повернення у основну програму
	    endp Whi  

          fnc proc near                 ;Процедурна реалізація функції
	    mov al,[bx]+len	 ;Читання елемента першого операнда 
	    or [bx],al                ;Функція дизюнкции
	                               
	    ret                               ;Повернення у основну програму
	    endp  fnc 
	    ...
	    jge rip		               ;Критерій ознаки порівняння X<=Y
	    ret                              ;Повернення у основну програму
	    endp rpi
...
scr_byte macro	   ;Видача байту ASCII Hexmal символів
	 ...
	 endm
	   ...
	   local next
	    mov cx,8           ;Ініціалізація лічильника станів
	    lea di, adr           ;та адреси
  ...
  sta:mov ax,@data		 ;Ініціалізація        
      ...
      cld                              ;Обробка стрічок уперед
     mes mes_9               ;Для позиціювання курсору
      mes mes_1 	  ;Повідомлення про початок тестування ознаки
      mes mes_2               ; Повідомлення 
