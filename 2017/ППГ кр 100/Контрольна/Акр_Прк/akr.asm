...
Z db len*2*4 dup (?)
    ...
    mes_4 db '              3 State after execute function X:=X or  Y',0dh,0ah, '$'
...
.code  ;Сегмент програми  
	   ...
	   local lop
	    ...
	    jl nx1		; Критерій ознаки порівняння Y >= X
	    inc byte ptr [bx]+len ; Дія обчисленняY:=Y - 1
	    jmp Whl		      
                nx1:ret 
	    endp Whi  
