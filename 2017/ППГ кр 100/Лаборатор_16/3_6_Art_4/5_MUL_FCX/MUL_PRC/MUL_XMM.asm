...
jmp mt1
  ...
  macro sym			  ;Вивід ASCII символу
      {push ax
       ...
       pop dx
       pop ax}
 mt1: out_str mes1				;TECHNOLOGI XMM  THE PENTIUM PROCESSOR
...
mulps xmm0,xmm7 		 ;MULtiplication Packed Single precesion float point
movups [edx],xmm0		 ;MOVe Unaligned four Packed Single precesion float point   
out_str mes4					     ; Packed_Product_Z: 
 scr_ddw Opr_Y				    
 cursor
     out_str mes5			  ;STOP TESTING  PENTIUM PROCESSOR
exit: mov ax,4c00h
      int 21h
Opr_X  DD  -199.1753E-7;
	     DD  -9421.1754E+3;
	     DD  3.4028234E+37;
	     DD  3.4025678E+27;
Opr_Y dd 16 dup (?)    ; Резервування в памяти 16-ти елементів  підд операнд Y
	mes1 db '            START TESTING XMM MULTIPLICATION THE PENTIUM PROCESSOR ',0ah,0dh,0ah,'$'
	mes2 db '       Packed_Operand_X: $'
	mes3 db '       Packed_Operand_Y: $'
	mes4 db '       Packed_Result_Z : $'
	mes5 db '            STOP TESTING XMM MULTIPLICATION THE PENTIUM PROCESSOR ',0ah,0dh,0ah,'$'
